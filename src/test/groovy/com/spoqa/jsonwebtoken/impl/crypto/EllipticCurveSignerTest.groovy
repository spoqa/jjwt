...
class EllipticCurveSignerTest {
    ...
    void testDoSignWithInvalidKeyException() {
        ...
        try {
            signer.sign(bytes)
            fail();
        } catch (com.spoqa.jsonwebtoken.SignatureException se) {
    ...
    void testDoSignWithJoseSignatureFormatException() {
        ...
        String msg = 'foo'
        final com.spoqa.jsonwebtoken.JwtException ex = new com.spoqa.jsonwebtoken.JwtException(msg)

        def signer = new EllipticCurveSigner(SignatureAlgorithm.ES256, privateKey) {
            @Override
            protected byte[] doSign(byte[] data) throws InvalidKeyException, java.security.SignatureException, com.spoqa.jsonwebtoken.JwtException {
                throw ex
            }
        }
